...
Use custom hooks for your contexts, so that you can check if the context is defined and throw a meaningful error if it is not.

This compound components share an implicit state.

Do set the context value to undefined, otherwise it is just wrong, especially if the value contains callback functions.

This pattern is used in a lot of frameworks, like material-ui, etc.
